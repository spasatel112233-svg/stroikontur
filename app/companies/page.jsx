    ...
    <main className="page">
      <div className="container">
        <PageHero
          ...
          actions={<><Link href="/request" className="btn btn-primary">Найти компанию</Link><Link href="/register" className="btn btn-secondary">Создать профиль</Link></>}
        />
          ...
          <div className="company-grid">
            {companies.map((company) => (
              <article className="company-card card" key={company.title}>
                <div className="avatar">{company.title.slice(0,2).toUpperCase()}</div>
                <h3 className="card-subtitle">{company.title}</h3>
                <p className="card-text">{company.text}</p>
                <div className="meta">{company.meta}</div>
                <div className="card-actions">
                  <Link href="/request" className="btn btn-secondary">Подробнее</Link>
                  <Link href="/request" className="btn btn-primary">Связаться</Link>
                </div>
              </article>
            ))}
          </div>
